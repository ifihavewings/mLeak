  ...
  doPost: function (params) {
    ...
    axiosInstance.interceptors.response.use(
      response => {
        ...
        // 3.1返回50160  不需要弹窗，直接返回response
        // 3.2返回50161  需要弹窗，直接返回response
        const rcLength = responseCode.length
        // debugger
        for (let i = 0; i < rcLength; i++) {
          if (responseCode[i]['code'] === data.respCode) {
            console.log(responseCode[i]['code'] === data.respCode)
            console.log(responseCode[i]['show'])
            // 不需要显示弹框
        ...
        let dialogContent = ''
        if ('' === data.respDesc || undefined === data.respDesc) {
